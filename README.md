...
- **OOP:** Classes, single inheritance, interfaces, visibility (`public` / `protected` / `private`), `abstract` / `final`, `readonly` classes/properties, `const` methods/parameters, constructors (`construct`) and destructors (`destruct`), `Cloneable` interface.
