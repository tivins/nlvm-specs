...
| **[docs/compiler.md](docs/compiler.md)** | **Compiler semantic analyses.** Definite assignment, null safety, type checking, immutability (`const` / `readonly`), exception checking, visibility, parameter validation, entry-point validation. Defines *what* the compiler must verify and reject (31 error codes, 1 warning). § [Compiler invocation (nlc)](docs/compiler.md#compiler-invocation-nlc) defines the compiler CLI. |
